...
#pragma warning disable CS8602
    internal class ChatTextBoxPatches
    {
        ...
        #region Clipboard
        [DllImport("SDL2")] private static extern IntPtr SDL_GetClipboardText();
        [DllImport("SDL2")] private static extern void SDL_free(IntPtr mem);
        ...
        private static string GetClipboard()
        {
            ...
            {
                ModEntry.Instance?.Monitor.Log($"Clipboard error: {ex.Message}", StardewModdingAPI.LogLevel.Error);
                return "";
            }
        }
        ...
        private static void SetClipboard(string text)
        {
            try
            {
                ...
                try { SDL_SetClipboardText_Internal(handle.AddrOfPinnedObject()); }
                finally { handle.Free(); }
            }
            catch (Exception ex)
            {
        ...
        private static int GetNextSegmentEnd(string text, int pos)
        {
            ...
            if (char.IsLetterOrDigit(c))
                return AdvanceWhile(text, pos, char.IsLetterOrDigit);

            return AdvanceWhile(text, pos, ch => !char.IsWhiteSpace(ch) && !char.IsLetterOrDigit(ch));
        }
        ...
        private static int GetPrevSegmentStart(string text, int pos)
        {
            ...
            if (char.IsLetterOrDigit(c))
                return RetreatWhile(text, pos - 1, char.IsLetterOrDigit);

            return RetreatWhile(text, pos - 1, ch => !char.IsWhiteSpace(ch) && !char.IsLetterOrDigit(ch));
        }
        ...
        {
            if (shift && s.SelectionStart == s.SelectionEnd)
        ...
        public class RecieveCommandInputPatch
        {
            private static bool Prefix(ChatTextBox __instance, char command)
            {
                ...
                var s = GetState(__instance);
                var keys = Game1.input.GetKeyboardState();
                ...
                else if (s.CursorIndex > 0)
                {
                    ...
                    {
                        var emoji = GetEmojiToDelete(s.FullText, s.CursorIndex, true);
                        if (emoji.start != -1)
                        {
        ...
        public class ReceiveLeftClickPatch
        {
            private static bool Prefix(ChatBox __instance, int x, int y)
            {
                ...
                if (__instance.choosingEmoji && __instance.emojiMenu.isWithinBounds(x, y))
                {
                    ...
                    var s = GetState(__instance.chatBox);
            ...
            private static int CalculateCursorFromClick(ChatBox chatBox, int x, TextBoxState s)
            {
                ...
                foreach (var snippet in msg.message)
                {
                    ...
                    else if (snippet.message != null)
                    {
                        ...
                        {
                            float snippetStartX = curX;
                            for (int i = 0; i < snippet.message.Length; i++)
                            {
                                float charEndX = snippetStartX + font.MeasureString(snippet.message[..(i + 1)]).X;
                                float charStartX = i > 0 ? snippetStartX + font.MeasureString(snippet.message[..i]).X : snippetStartX;
                ...
                switch (key)
                {
                    ...
                    case Keys.Delete:
                        ...
                        else if (s.CursorIndex < s.FullText.Length)
                        {
                            ...
                            {
                                var emoji = GetEmojiToDelete(s.FullText, s.CursorIndex, false);
        ...
        public class TextBoxEnterPatch
        {
            private static bool Prefix(ChatBox __instance, TextBox sender)
            {
                ...
                if (s.FullText.Length > ModEntry.Instance.Config.MaxMessageLength)
                {
        ...
        public class DrawPatch
        {
            ...
            private static bool Prefix(ChatTextBox __instance, SpriteBatch spriteBatch, bool drawShadow)
            {
                ...
                var tex = _textBoxTexture(__instance);
                var color = _textColor(__instance);
                ...
                if (tex != null)
                {
                    ...
                    spriteBatch.Draw(tex, new Rectangle(__instance.X + 16, __instance.Y, __instance.Width - 32, __instance.Height),
                        new Rectangle(16, 0, 4, __instance.Height), Color.White);
                    spriteBatch.Draw(tex, new Rectangle(__instance.X + __instance.Width - 16, __instance.Y, 16, __instance.Height),
                    ...
                    Game1.drawDialogueBox(__instance.X - 32, __instance.Y - 112 + 10, __instance.Width + 80,
                        __instance.Height, speaker: false, drawOnlyBox: true);
                ...
                var oldRaster = spriteBatch.GraphicsDevice.RasterizerState;
                spriteBatch.End();

                var raster = new RasterizerState { ScissorTestEnable = true };
                var scissor = new Rectangle(__instance.X + 16, __instance.Y, __instance.Width - 72, __instance.Height);
        ...
        public class UpdatePatch
        {
            private static void Postfix(ChatBox __instance, GameTime time)
            {
                ...
                if (s.WasMousePressed && !isMousePressed)
                {
                    s.IsDragging = false;
                }
            ...
            Found:
                return ChatTextBoxPatches.SnapCursorToEmojiBoundary(s.FullText, newCursor, 0);
            }
